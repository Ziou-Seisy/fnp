...
var FnpCompilerDefine = {
	sign:[
		...
		{type:"Math_inline",beg: "\\${",end:"}\\$"},
		{type:"Math_inline",beg: "\\M{",end:"}\\M"},
		{type:"Math_line",beg: "\r$ ",end:"\n"},
		{type:"Math_line",beg: "\r#M ",end:"\n"},
		{type:"Math_box",beg: "\r$.beg\n",end:"\r$.end\n"},
		{type:"Math_token",beg: "\r$Token ",end:"\n"},
	],
	toObj:{
		H1: ["@Basic"],
		H2: ["@Basic"],
	...
	function once_has_df_rc(FnpCompilerDef){
		...
		for (let k in FnpCompilerDef.toObj) {
			...
			if(typeof df[func[0]] !== 'function'){
				console.error("Fnp > 意外！不是函数：", df[func[0]],func,df)
				return
			}
...
var FnpInterpreterDefine = {
	toHTML:{
		...
		code_mul:["#ElementNesting",["#ElementArray","code","Hcode",{},{},2,2],["#ElementArray","pre","code_mul",{},{className:{t:"&{text}",i:1}}]],
...
var fnp_user = {}
function HTMLTextRset(text,u1=0){
	let p = text.replaceAll('\n\r','\u0001').replaceAll('\n','\u0001').replaceAll('\r','').replaceAll('\u0001','\r\n');
	return u1? p : p.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;')
}
	...
	function fset(FnpInterpreterDef, doctype){
		...
		var df = {
			...
			"#ElementArray":(tag, className, obj={}, objRep={}, text_i=0,text_pos=0)=>(arg)=>{
				//console.log(tag, className, obj, objRep, text_i,text_pos)
				try{
				...
				if(Array.isArray(arg[text_i-1])){
					...
					//console.log(ele,arg[text_i-1],text_pos)
					if(text_pos == 0)ele.innerText = arg[text_i-1]
					else if(text_pos == 1){
						ele.innerHTML = arg[text_i-1].replaceAll('\n\r','<br/>')
					}
					else if(text_pos == 2){
						ele.innerText = HTMLTextRset(arg[text_i-1],1)
					}
					else console.error("Fnp > 意外！没有选项", text_pos, tag)
				}
